...
<c:if test='${null == param.rights || "" == param.rights.trim()}'>
    ...
    <fmt:message bundle="${messages}" key="rights.edit.name.empty" 
                 scope="request" var="rights_name_error"/>
</c:if>

<c:if test='${null == param.deleted || "" == param.deleted.trim()}'>
    <c:set scope="page" value="false" var="proceed"/>
    <fmt:message bundle="${messages}" key="rights.edit.status.empty" 
...
<c:if test="${proceed}">
    ...
    <c:if test="${null == error}">
        <c:forEach items="${result.rows}" var="row">
            <fmt:message bundle="${messages}" key="rights.edit.name.exist"
...
<c:if test="${proceed}">
    ...
    <c:if test="${null == error}">
        <fmt:message bundle="${messages}" key="rights.edit.success" scope="request"
                     var="success_msg"/>
